...
function zoomIndexScale(scale, zoom, center, zoomOptions) {
	...
	var zoomStep = Math.round((maxIndex-minIndex)*0.1);
	// console.log("1. minIndex = " + minIndex);
	// console.log("2. maxIndex = " + maxIndex);
	//console.log("3. zoomStep = " + zoomStep);
	if (Math.abs(zoomNS.zoomCumulativeDelta) > sensitivity){
		if(zoomNS.zoomCumulativeDelta < 0){
			if(centerPointer >= chartCenter){
				if (minIndex <= 0){
					maxIndex = Math.min(lastLabelIndex, maxIndex + zoomStep);
				} else{
					minIndex = Math.max(0, minIndex - zoomStep);
				}
			...
			if(centerPointer >= chartCenter){
				minIndex = minIndex < maxIndex ? minIndex = Math.min(maxIndex, minIndex + zoomStep) : minIndex;
			} else if(centerPointer < chartCenter){
				maxIndex = maxIndex > minIndex ? maxIndex = Math.max(minIndex, maxIndex - zoomStep) : maxIndex;
			}
			zoomNS.zoomCumulativeDelta = 0;
		}
...
function panIndexScale(scale, delta, panOptions) {
	...
	var maxIndex;
	var sp = 1;//Math.round(1+offsetAmt*0.03);
	 
	zoomNS.panCumulativeDelta += delta;
	// console.log("1. delta = "+delta);
	// console.log("2. panCumlativeDelta = "+zoomNS.panCumulativeDelta);
	// console.log("3. offsetAmt = "+offsetAmt);
	// console.log("4. panSpeed = "+panSpeed);
	// console.log("5. step = "+step);
	// console.log("-------------------");
	// console.log("before minIndex = "+minIndex);
	minIndex = zoomNS.panCumulativeDelta > step ? Math.max(0, minIndex -sp) : zoomNS.panCumulativeDelta < -step ? Math.min(lastLabelIndex - offsetAmt + sp, minIndex + sp) : minIndex;
	// console.log("after minIndex = "+minIndex);
	// console.log("-------------------");
	...
	if (panOptions && helpers.getValueOrDefault(panOptions.enabled, defaultOptions.pan.enabled)) {
		...
		helpers.each(chartInstance.scales, function(scale, id) {
			if (scale.isHorizontal() && directionEnabled(panMode, 'x') && deltaX !== 0) {
				...
				panScale(scale, deltaX*100, panOptions);				
